...
# --- 3. DICCIONARIO MULTILINGÜE (Inclusión) ---
textos = {
    "Español": {
...
# --- 5. BARRA LATERAL (PANEL DE CONTROL PROFESIONAL) ---
with st.sidebar:
...
# --- 6. ESTILOS CSS (Efectos de Impulsa + Flowmerce) ---
extra_styles = ""
...
with tab1:
    ...
    col1.metric(t_act["atrapado"], f"${atrapado_val:,.0f} MXN", help="Dinero estancado en stock que no rota")
    col2.metric(t_act["riesgo"], f"${riesgo_val:,.0f} MXN", delta="¡Alerta!", delta_color="inverse")
...
with tab2:
    ...
    if st.button("🚀 Ejecutar Órdenes en Tiendanube"):
        with st.status("Sincronizando con API..."):
            time.sleep(2)
        st.balloons()
        st.success("Órdenes de compra generadas y cupones de liquidación creados.")

...
<style>
    ...
    .main-title {{ background: linear-gradient(90deg, #0056ff, #00c6ff, #6200ea, #0056ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 4rem; font-weight: 800; }}
    .team-card-large {{ text-align: center; padding: 25px; border-radius: 25px; background: rgba(255, 255, 255, 0.8); border: 1px solid #ddd; margin-bottom: 20px; }}
    .problem-box {{ background: white; padding: 20px; border-radius: 15px; border-left: 8px solid #0056ff; box-shadow: 0 4px 6px rgba(0,0,0,0.1); height: 100%; }}
    {extra_styles}
</style>
...
if audio:
    ...
    # Simulación de reconocimiento para el demo
    transcripcion = "Optimizar ventas y revisar stock"
    with st.chat_message("user"):
        st.write(f"🎙️ **Voz reconocida:** '{transcripcion}'")
    with st.chat_message("assistant"):
        st.write("🤖 Entendido. Analizando tu tienda ahora mismo...")
    ...
    with col_left:
        ...
        if st.button("🎯 Ejecutar Optimización Operativa"):
            with st.status("Procesando...") as s:
                time.sleep(1)
                s.update(label="Sincronizando...", state="complete")
            st.balloons()

    with col_right:
        st.markdown("### 💬 Consulta IA")
        if st.button("Analizar"):
            st.info("📊 Producto 'Playera Algodón' es el más rentable.")
...
with tab_ins:
    ...
    with c2: st.markdown('<div class="problem-box"><h4>AIO / SEO</h4><p>Optimización para IAs.</p></div>', unsafe_allow_html=True)
        ...
        for j, (nombre, cargo, img_url) in enumerate(equipo[i:i+3]):
            with cols[j]:
                ...
                <div class="team-card-large">
                    <img src="{img_url}" style="width: 150px; height: 150px; border-radius: 50%; object-fit: cover; border: 5px solid #0056ff; margin-bottom: 15px;">
                    <br><strong>{nombre}</strong><br><span style="color: #0056ff;">{cargo}</span>
                </div>
                """, unsafe_allow_html=True)

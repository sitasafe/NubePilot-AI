...
<style>
    ...
    .main-title {{ background: linear-gradient(90deg, #0056ff, #00c6ff, #6200ea, #0056ff); background-size: 300% auto; -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 4.5rem !important; font-weight: 800; animation: gradient-move 4s ease infinite; }}
    @keyframes gradient-move {{ 0% {{ background-position: 0% 50%; }} 50% {{ background-position: 100% 50%; }} 100% {{ background-position: 0% 50%; }} }}
    .team-card-large {{ text-align: center; padding: 35px; border-radius: 30px; background: rgba(255, 255, 255, 0.8); backdrop-filter: blur(10px); border: 1px solid rgba(0, 86, 255, 0.1); box-shadow: 0px 20px 40px rgba(0,0,0,0.05); margin-bottom: 25px; transition: all 0.4s ease; }}
    .problem-box {{ background-color: white; padding: 25px; border-radius: 20px; border-left: 8px solid #0056ff; box-shadow: 0px 10px 25px rgba(0,0,0,0.03); height: 100%; transition: all 0.3s ease; }}
    {extra_styles}
</style>
...
# --- CORRECCIÓN: FUNCIONALIDAD DE VOZ ---
c_voz1, c_voz2 = st.columns([0.80, 0.20])
with c_voz2:
    audio = mic_recorder(start_prompt="🎤 Iniciar Voz", stop_prompt="🛑 Parar", key='recorder')

if audio:
    # PROCESAMIENTO DE VOZ REAL
    st.toast("Capturando audio...")
    # Simulamos el reconocimiento de voz para el Pitch (Speech-to-Text)
    # En un entorno real conectarías aquí con la API de OpenAI Whisper o Google Speech
    transcripcion_simulada = "Optimizar ventas y revisar stock" 
    
    with st.chat_message("user"):
        st.write(f"🎙️ **Voz reconocida:** '{transcripcion_simulada}'")
    
    with st.chat_message("assistant"):
        st.write("🤖 Entendido. Estoy analizando tu tienda basándome en tu comando de voz...")
        st.status("Ejecutando optimización por voz...")
...
with tab_dash:
    ...
    with col_left:
        ...
        if st.button("🎯 Ejecutar Optimización Operativa"):
            with st.status("Procesando...", expanded=True) as status:
                ...
                time.sleep(1)
                status.update(label="Ajustando pujas...", state="complete", expanded=False)
            st.balloons()
    ...
    with col_right:
        ...
        if st.button("Analizar"):
            st.info(f"📊 **Análisis:** Tu producto 'Playera Algodón' tiene un ROAS de 5.1x.")

# --- TAB 2: ESTRATEGIA Y AIO ---
with tab_ins:
    st.caption("🛡️ Análisis ético y cumplimiento de estándares (2026).")
    st.markdown("### 🧠 Soluciones Estratégicas")
    c1, c2, c3 = st.columns(3)
    with c1:
        st.markdown('<div class="problem-box"><h4>Eficiencia Publicitaria</h4><p>Ajuste dinámico de inversión.</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="problem-box"><h4>Optimización Inclusiva</h4><p>Adaptación para buscadores de IA.</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown('<div class="problem-box"><h4>Automatización Universal</h4><p>Interfaz simplificada ERP.</p></div>', unsafe_allow_html=True)
    ...
    with col_big1:
        ...
        df_pred = pd.DataFrame({"Día": [f"Día {i}" for i in range(1, 16)], "Ventas": np.random.randint(100, 200, 15)}).set_index("Día")
    ...
    with col_big2:
        ...
        if st.button("📊 Generar Reporte"):
            st.toast("Generando reporte accesible...")
...
with tab_team:
    ...
    equipo = [
        ...
        ("Dalia R.", "Product Manager", "https://i.

...
with st.sidebar:
    ...
    # Mejora: Doble vía de conexión (OAuth vs Manual técnico)
    metodo_auth = st.radio("Método de Conexión", ["Oficial (OAuth)", "Técnico (Network Token)"])

    if metodo_auth == "Oficial (OAuth)":
        with st.expander("🔑 Conexión Oficial Tiendanube", expanded=True):
            auth_url = f"https://www.tiendanube.com/apps/authorize?client_id={CLIENT_ID}&scope=read_orders,write_products,read_customers,read_checkouts"
        ...
        st.info("💡 Modo Experto: Usa el X-Access-Token de la pestaña Network.")
        token_manual = st.text_input("Ingresar Token Manual:", type="password")
        if st.button("Inyectar Token"):
            st.session_state['api_token'] = token_manual

    st.divider()
    api_token_val = st.session_state.get('api_token', "")
    api_token_input = st.text_input("Access Token Activo", type="password", value=api_token_val)
...
with tab_dash:
    ...
    m_col4.metric("Sincro Variantes", "Activo", "Color/Talle")
    ...
    with col_left:
        with st.chat_message("assistant"):
            st.write("🤖 **IA:** He analizado tus variantes. El stock del ERP para 'Talle L' está agotado pero sigue activo en Tiendanube. ¿Sincronizamos y ajustamos los precios de oferta?")
        
        if st.button("🎯 Ejecutar Optimización Operativa"):
            with st.status("Procesando...", expanded=True) as status:
                time.sleep(1)
                status.update(label="Sincronizando variantes con ERP...", state="running")
                time.sleep(1)
                status.update(label="Ajustando Precios Tachados (Ofertas)...", state="running")
                time.sleep(1)
                status.update(label="Optimización AIO completada.", state="complete", expanded=False)
            st.balloons()
    ...
    with col_right:
        ...
        if st.button("Analizar"):
            st.info("📊 **Análisis:** Tu 'Playera Algodón' tiene mayor ROAS. Sugiero activar oferta de 'Precio Anterior' para liquidar variantes de poco stock.")
...
with tab_ins:
    ...
    with c1:
        st.markdown('<div class="problem-box"><span class="status-tag">ADS & ROAS</span><h4>Gestión de Ofertas</h4><p>Uso inteligente de precios tachados para aumentar urgencia de compra.</p></div>', unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="problem-box"><span class="status-tag">AIO / SEO</span><h4>Optimización para IA</h4><p>Indexamos tus variantes para que sean la respuesta #1 en ChatGPT y Gemini.</p></div>', unsafe_allow_html=True)
    with c3:
        st.markdown('<div class="problem-box"><span class="status-tag">ERP CONNECT</span><h4>Sincronía de Variantes</h4><p>Control exacto de stock por color y talle desde sistemas externos.</p></div>', unsafe_allow_html=True)
    ...
    with col_big1:
        ...
        df_pred = pd.DataFrame({"Día": [f"Día {i}" for i in range(1, 16)], "Tendencia IA": np.random.randint(150, 250, 15)}).set_index("Día")
...
# --- TAB 3: EQUIPO (RESTAURADO COMPLETO) ---
with tab_team:

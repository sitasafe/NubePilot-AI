...
def obtener_token_real(code):
    ...
    headers = {
        ...
        "User-Agent": "GrowthCopilot (socios@tiendanube.com)"
    }
    payload = {
    ...
    with st.expander("🔑 Conexión Oficial Tiendanube", expanded=True):
        ...
        if st.button("3. Vincular Tienda"):
            ...
            if token_valido:
                ...
                st.success("¡Conexión Real Establecida con Growth Copilot! ✅")
            else:
...
with tab_ins:
    ...
    with col_big2:
        ...
        if st.button("📊 Generar Reporte de Big Data"):
            ...
            time.sleep(1)
            st.download_button("Descargar Plan Accesible PDF", data="Contenido inclusivo...", file_name="Plan_GrowthCopilot_Inclusivo.txt")

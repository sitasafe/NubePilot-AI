...
<style>
    ...
    div[data-testid="stMetric"], .team-card-large, div[data-testid="stExpander"], .stDataEditor {{ background-color: white !important; border-radius: 15px !important; border: none !important; padding: 20px !important; box-shadow: 0 4px 15px rgba(0,0,0,0.05) !important; }}
</style>
""", unsafe_allow_html=True)
...
with tabs[2]:
    ...
    # 1. Tabla Editable (Editor de Datos)
    st.markdown("#### 📝 Gestión de Stock e Inventario")
    st.caption("Cambia los números en 'Stock' o 'Ventas_30d' y presiona Enter para ver la nueva Acción.")
    
    # Preparamos la columna de Acción antes de mostrarla
    def asignar_label(autonomia):
        if autonomia < dias_entrega: return "🚨 REABASTECER"
        if autonomia > 60: return "🔥 LIQUIDAR"
        return "✅ ESTABLE"
    
    df["Accion"] = df["Autonomia"].apply(asignar_label)
    
    # El editor que permite cambiar números y actualiza todo
    df_editado = st.data_editor(
        st.session_state.db_inventario,
        column_config={
            "Stock": st.column_config.NumberColumn("Stock Actual", help="Unidades en bodega", min_value=0),
            "Ventas_30d": st.column_config.NumberColumn("Ventas 30d", min_value=0),
            "Costo": st.column_config.NumberColumn("Costo Unitario ($)", min_value=0)
        },
    ...
    # Si hay cambios, guardamos en memoria y recargamos para actualizar las acciones
    if not df_editado.equals(st.session_state.db_inventario):
        st.session_state.db_inventario = df_editado
        st.rerun()

    # Mostrar la tabla de Acciones calculadas (Igual a tu imagen)
    st.markdown("#### 🎯 Acciones Sugeridas")
    st.dataframe(df[["Producto", "Stock", "Accion"]], use_container_width=True, hide_index=True)

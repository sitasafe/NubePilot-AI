...
# --- 5. GESTIÓN DE MEMORIA (ESTADO INICIAL) ---
...
# --- 7. LÓGICA DE CÁLCULO CENTRAL ---
t_act = textos[idioma]
df_actual = st.session_state.db_inventario.copy()

# Cálculo de autonomía y métricas basado en los datos actuales de la sesión
...
atrapado_val = (df_actual[df_actual["Autonomia"] > 60]["Stock"] * df_actual[df_actual["Autonomia"] > 60]["Costo"]).sum()
riesgo_val = (df_actual[df_actual["Autonomia"] < dias_entrega]["V_Diaria"] * df_actual[df_actual["Autonomia"] < dias_entrega]["Costo"] * 1.5).sum()
...
<style>
    ...
    .main-title {{ background: linear-gradient(90deg, #0056ff, #00c6ff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; font-size: 3rem; font-weight: 800; }}
    div[data-testid="stMetric"], .team-card-large, .stDataEditor {{ background-color: white !important; border-radius: 15px !important; padding: 15px !important; box-shadow: 0 4px 10px rgba(0,0,0,0.05) !important; }}
</style>
""", unsafe_allow_html=True)
...
with tabs[1]:
    ...
    st.area_chart(df_actual.set_index("Producto")["Stock"])
...
with tabs[2]:
    ...
    with st.expander(t_act["sim_tit"], expanded=True):
        ...
        with c_s1: st.metric(t_act["sim_proj"], f"${sim_inv * (f_demanda * 1.8):,.0f} MXN")
        with c_s2: st.metric(t_act["sim_rec"], f"{30/f_demanda:.1f} {t_act['sim_dias']}")

    st.write("---")
    st.markdown("### 📝 Gestión Dinámica de Inventario")
    
    # TABLA INTERACTIVA: Al editar aquí, se guarda en st.session_state
    df_editor = st.data_editor(
        st.session_state.db_inventario,
        column_config={
            "Producto": st.column_config.TextColumn("Producto", disabled=True),
            "Stock": st.column_config.NumberColumn("Stock", min_value=0),
            "Ventas_30d": st.column_config.NumberColumn("Ventas 30d", min_value=0),
    ...
    # RE-CÁLCULO AUTOMÁTICO: Si los datos del editor son distintos a la memoria, actualizamos y recargamos
    if not df_editor.equals(st.session_state.db_inventario):
        st.session_state.db_inventario = df_editor
        st.rerun() # Esto hace que los cálculos de arriba (atrapado/riesgo) se refresquen al instante

    col_b1, col_b2 = st.columns(2)
    with col_b1:
        if st.button(t_act["btn_app"], use_container_width=True):
            st.success("📦 Sincronizado con Tiendanube")
    with col_b2:
        csv = df_editor.to_csv(index=False).encode('utf-8')
        st.download_button(t_act["btn_reporte"], data=csv, file_name='Plan_Flowmerce.csv', use_container_width=True)
...
with tabs[3]:
    ...
    for j, (nombre, cargo, img) in enumerate(equipo):
        with cols[j]:
            st.markdown(f'<div class="team-card-large" style="text-align:center;"><img src="{img}" width="100" style="border-radius:50%;"><br><b>{nombre}</b><br>{cargo}</div>', unsafe_allow_html=True)

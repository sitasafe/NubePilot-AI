...
<style>
    ...
    div[data-testid="stMetric"], .stTable, .team-card-large, div[data-testid="stExpander"], .stDataEditor {{ background-color: white !important; border-radius: 15px !important; padding: 20px !important; box-shadow: 0 4px 15px rgba(0,0,0,0.05) !important; }}
...
# --- 8. LÓGICA DE CÁLCULO DINÁMICO ---
t_act = textos[idioma]

# Función para aplicar cálculos al dataframe
def calcular_metricas(dataframe):
    temp_df = dataframe.copy()
    temp_df["V_Diaria"] = (temp_df["Ventas_30d"] / 30) * f_demanda
    temp_df["Autonomia"] = np.where(temp_df["V_Diaria"] > 0, temp_df["Stock"] / temp_df["V_Diaria"], 999)
    atrapado = (temp_df[temp_df["Autonomia"] > 60]["Stock"] * temp_df[temp_df["Autonomia"] > 60]["Costo"]).sum()
    riesgo = (temp_df[temp_df["Autonomia"] < dias_entrega]["V_Diaria"] * temp_df[temp_df["Autonomia"] < dias_entrega]["Costo"] * 1.5).sum()
    return temp_df, atrapado, riesgo

# Realizar cálculos iniciales con lo que haya en session_state
df_calculado, atrapado_val, riesgo_val = calcular_metricas(st.session_state.db_inventario)
...
with tabs[1]:
    # Estas métricas se actualizan solas al cambiar la tabla abajo
    ...
    st.area_chart(df_calculado.set_index("Producto")["Stock"])
...
with tabs[2]:
    ...
    st.write("---")
    st.markdown("### 📝 Editor de Inventario Inteligente")
    st.caption("Cambia los valores aquí y mira cómo se actualiza el 'Monitor de Liquidez'")

    # --- LA PARTE CLAVE: EDITOR DE DATOS ---
    df_editor = st.data_editor(
        ...
        column_config={
            ...
            "Costo": st.column_config.NumberColumn("Costo Unit.", format="$%d"),
        },
    ...
    # Si detecta cambios en la tabla, actualiza la sesión y recarga para mostrar los nuevos cálculos
    ...
    with col_b2:
        ...
        st.download_button(label=t_act["btn_reporte"], data=csv, file_name='Flowmerce_Data.csv', use_container_width=True)
